...
import { BookOpen, Mic, Play, Sparkles, LayoutGrid, Loader2 } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { Progress } from "@/components/ui/progress"
import { ScrollArea } from "@/components/ui/scroll-area"
import { fetchLibraryBooks, fetchBookManagementDetail } from "@/lib/queries"
import type { LibraryBook, ManagementBookDetail } from "@/lib/types"
import { cn } from "@/lib/utils"

export default function ManagementPage() {
  const [books, setBooks] = useState<LibraryBook[]>([])
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null)
  const [selectedBook, setSelectedBook] = useState<ManagementBookDetail | null>(null)
  ...
  useEffect(() => {
    ...
    fetchBookManagementDetail(selectedSlug)
      .then((data) => {
    ...
    <div className="container mx-auto px-4 py-12 max-w-7xl">
      ...
      {loadingBooks ? (
        <div className="flex flex-col items-center justify-center py-24 text-muted">
          ...
          <p className="text-sm font-medium">Loading scripture books...</p>
        </div>
      ) : books.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-muted bg-white border border-border/40 rounded-3xl p-10 max-w-md mx-auto mt-10 shadow-premium">
          ...
          <p className="text-sm mt-1 text-center">Your library is empty. Add a manuscript to start managing chapters.</p>
          <Link href="/upload">
        ...
        <div className="grid gap-8 lg:grid-cols-4">
            ...
            <div className="rounded-2xl border border-border/40 bg-white p-5 shadow-premium">
              ...
              <ScrollArea className="h-[580px] pr-2">
                <div className="space-y-1.5">
                  {books.map((book) => {
                    const active = selectedSlug === book.slug
                    return (
                      <button
                        key={book.id}
                        onClick={() => setSelectedSlug(book.slug)}
                        className={cn(
                          "w-full flex items-center gap-3 rounded-xl p-3 text-left transition-all border cursor-pointer",
                          active
          ...
          >
            {loadingDetail || !selectedBook ? (
              <div className="rounded-2xl border border-border/40 bg-white p-6 md:p-8 shadow-premium flex flex-col items-center justify-center py-24 text-muted">
                <Loader2 className="h-10 w-10 mb-4 animate-spin text-primary" />
              ...
              <div className="rounded-2xl border border-border/40 bg-white p-6 md:p-8 shadow-premium">
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/20 pb-5">
                  ...
                  <Link href={`/book/${selectedBook.slug}`}>
                ...
                <ScrollArea className="h-[550px] pr-2">
                  <div className="space-y-4">
                    {selectedBook.chapters.map((chapter, ci) => {
                      const chapterCompletion = chapter.verses.length > 0
                        ? Math.round((chapter.recordedCount / chapter.verses.length) * 100)
                        : 0
                        ...
                        >
                          <div className="flex items-center justify-between mb-3.5">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <Link href={`/recording/${selectedBook.slug}/${chapter.number}`}>
                          ...
                          <div className="flex flex-wrap gap-1 border-t border-border/20 pt-3">
                            {chapter.verses.slice(0, 48).map((verse) => (
                              <div
                                key={verse.id}
                                className={cn("h-2 w-2 rounded-full", verse.recorded ? "bg-accent" : "bg-muted/30")}
                                title={`Verse ${verse.number}: ${verse.recorded ? "Recorded" : "Pending"}`}
                              />
                            ))}
                            {chapter.verses.length > 48 && (
                              <span className="text-[10px] font-bold text-muted ml-2">+{chapter.verses.length - 48} verses</span>
                            )}
                          </div>
                        </motion.div>
                      )
