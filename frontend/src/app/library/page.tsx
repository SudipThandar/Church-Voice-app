...
import { Search, BookOpen, Globe, User, Layers, Sparkles, Loader2 } from "lucide-react"
    ...
    <div className="container mx-auto px-4 py-12 max-w-7xl">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 text-muted">
          ...
          <p className="text-sm font-medium">Loading scripture library...</p>
        </div>
      ) : (
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filtered.map((book, i) => (
            ...
            >
              <Link href={`/book/${book.slug}`} className="group block h-full">
      ...
      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-muted bg-white border border-border/40 rounded-3xl p-10 max-w-md mx-auto mt-10 shadow-premium">
          ...
          <p className="text-sm mt-1 text-center">
            {books.length === 0
              ? "The library is empty. Scripture volumes will appear here once added."
