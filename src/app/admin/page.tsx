...
import { books, analyticsData } from "@/lib/mock-data"
...
export default function AdminDashboard() {
  ...
  const avgCompletion = Math.round(books.reduce((sum, b) => sum + b.completion, 0) / books.length)
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-7xl">
      ...
      >
        ...
        <h1 className="text-4xl font-bold text-foreground font-serif mt-3">Admin Dashboard</h1>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard
          ...
          value={analyticsData.totalRecordings.toLocaleString()}
          color="green"
          trend="+12%"
          delay={0.15}
        />
      ...
      <motion.div
        ...
        className="mt-10 rounded-3xl border border-border/40 bg-card p-6 md:p-8 shadow-premium"
      >
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-base font-bold text-foreground group-hover:text-primary transition-colors">{book.title}</span>
      ...
      >
        <Link href="/analytics" className="block">
          <div className="rounded-2xl border border-border/40 bg-card p-6 hover:border-primary/20 hover:shadow-premium hover:-translate-y-1 transition-all duration-300 h-full">
            <TrendingUp className="h-9 w-9 text-primary mb-4" />
            <h3 className="font-bold text-foreground text-lg font-serif">Active Observers</h3>
            <p className="text-3xl font-extrabold text-primary mt-2 font-sans">{analyticsData.activeListeners.toLocaleString()}</p>
        ...
        <Link href="/analytics" className="block">
          <div className="rounded-2xl border border-border/40 bg-card p-6 hover:border-accent/30 hover:shadow-premium hover:-translate-y-1 transition-all duration-300 h-full">
            <Clock className="h-9 w-9 text-accent mb-4" />
            <h3 className="font-bold text-foreground text-lg font-serif">Audio Run hours</h3>
            <p className="text-3xl font-extrabold text-accent mt-2 font-sans">{analyticsData.listeningHours.toLocaleString()}</p>
        ...
        <Link href="/management" className="block">
          <div className="rounded-2xl border border-border/40 bg-card p-6 hover:border-emerald-500/20 hover:shadow-premium hover:-translate-y-1 transition-all duration-300 h-full">
            <BookCheck className="h-9 w-9 text-emerald-500 mb-4" />
            <h3 className="font-bold text-foreground text-lg font-serif">Overall Completeness</h3>
