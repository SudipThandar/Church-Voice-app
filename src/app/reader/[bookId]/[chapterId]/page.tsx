...
import { ArrowLeft, Play, BookOpen, Music, Clock, Settings, Type, Sparkles, Volume2 } from "lucide-react"
...
export default function ReaderPage() {
  ...
  const book = getBookById(params.bookId as string)
  ...
  // Sync active verse and trigger smooth scrolling
  useEffect(() => {
    if (
      playerState.isPlaying &&
      playerState.track?.bookTitle === book?.title &&
      playerState.track?.chapterTitle === chapter?.title &&
      playerState.track?.verseNumber
    ) {
      const playingVerseNum = playerState.track.verseNumber
  ...
  }, [playerState.isPlaying, playerState.track?.verseNumber, playerState.track?.chapterTitle, book?.title, chapter?.title])
  ...
  const handlePlayVerse = (verse: typeof chapter.verses[0]) => {
  ...
  const isVersePlaying = (v: typeof chapter.verses[0]) =>
    playerState.isPlaying &&
    playerState.track?.verseNumber === v.number &&
    playerState.track?.chapterTitle === chapter.title &&
    playerState.track?.bookTitle === book.title
      ...
      <div className="container mx-auto max-w-4xl px-4 py-10 pb-28">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Badge variant="outline" className="text-xs gap-1.5 px-3 py-1 bg-white/40 backdrop-blur-sm border-border/40">
              <Clock className="h-3.5 w-3.5 text-primary" /> {formatDuration(chapter.duration)}
            </Badge>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {(["sm", "base", "lg", "xl", "2xl"] as FontSize[]).map((size) => (
                    <button
                      ...
                      className={`flex-1 min-w-[32px] h-8 rounded-lg text-xs font-semibold border transition-all ${
                        fontSize === size
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <Button
              className="gap-2 bg-accent text-dark hover:bg-accent/90 font-bold px-6 h-11 rounded-xl shadow-md cursor-pointer shrink-0"
                    ...
                    className={`w-full text-left transition-all duration-300 rounded-2xl p-4 md:p-5 border border-transparent ${
                      playing 
                        ...
                        : active 
                          ...
                          : "opacity-45 hover:opacity-85 hover:bg-black/5"
                    }`}
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        {/* Scripture body text */}
                        <p className={`font-serif tracking-wide text-justify text-dark ${fontClasses[fontSize]} ${
                    ...
                    {/* Progress tracking line for the active verse */}
                    <AnimatePresence>
                      {playing && (
                        <motion.div
                          initial={{ width: "0%" }}
                          animate={{ width: "100%" }}
                          exit={{ width: "0%" }}
                          transition={{ duration: verse.duration, ease: "linear" }}
                          className="absolute bottom-0 left-0 h-0.75 bg-accent rounded-full"
                        />
                      )}
                    </AnimatePresence>
                  </button>
