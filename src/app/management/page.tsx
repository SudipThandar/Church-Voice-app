...
import { BookOpen, Mic, Play, Sparkles, LayoutGrid, CheckCircle } from "lucide-react"
          ...
          <div className="rounded-2xl border border-border/40 bg-card p-6 md:p-8 shadow-premium">
            ...
            <ScrollArea className="h-[550px] pr-2">
              <div className="space-y-4">
                {selectedBook.chapters.slice(0, 15).map((chapter, ci) => {
                  // Determine constant deterministic completion percentage for demo
                  const chapterCompletion = Math.round(
                    chapter.verses.filter((_, idx) => (idx + ci) % 3 !== 0).length / chapter.verses.length * 100
                  )
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Verse mini progress layout */}
                      <div className="flex flex-wrap gap-1 border-t border-border/20 pt-3">
                        {chapter.verses.slice(0, 24).map((verse, idx) => {
                          const recorded = (idx + ci) % 3 !== 0
                          return (
                            <div
                              key={verse.id}
                              className={cn(
                                "h-2 w-2 rounded-full transition-all duration-300",
                                recorded ? "bg-emerald-400 shadow-sm shadow-emerald-500/20" : "bg-muted/30"
                              )}
                              title={`Verse ${verse.number}: ${recorded ? "Recorded" : "Pending"}`}
                            />
                          )
                        })}
